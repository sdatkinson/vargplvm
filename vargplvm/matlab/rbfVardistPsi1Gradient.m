function [gKern, gVarmeans, gVarcovars, gInd] = rbfVardistPsi1Gradient(rbfKern, vardist, Z, covGrad, learnInducing)

% RBFVARDISTPSI1GRADIENT description.
...
[K_fu, Knovar] = rbfVardistPsi1Compute(rbfKern, vardist, Z);

% inverse variances
A = zeros(1, vardist.latentDimension) + rbfKern.inverseWidth;
...
%
    ...
    % #HACK not sure if I converted this correctly from ARD2...
    % (used to be gKernlengcs(q) = -0.5 * ...)
    gKernlengcs = gKernlengcs - 0.5*sum(sum(B1_q.*KfuCovGrad)); 
