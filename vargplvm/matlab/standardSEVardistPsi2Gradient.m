function [gKern, gVarmeans, gVarcovars, gInd] = ...
    standardSEVardistPsi2Gradient(sKern, vardist, Z, covGrad, ...
    learnInducing)

% STANDARDSEVARDISTPSI2GRADIENT description.
...
if pool_open && (isfield(vardist,'parallel') && vardist.parallel) && size(vardist.means,1) > 15
    [gKern, gVarmeans, gVarcovars, gInd] = rbfard2VardistPsi2GradientPar(sKern, vardist, Z, covGrad, learnInducing);
else
    [gKern, gVarmeans, gVarcovars, gInd] = rbfard2VardistPsi2GradientOrig(sKern, vardist, Z, covGrad, learnInducing);
end
...
function [gKern, gVarmeans, gVarcovars, gInd] = rbfard2VardistPsi2GradientPar(sKern, vardist, Z, covGrad, learnInducing)
% variational means
...
[~, outKern, sumKern, Kgvar] = rbfard2VardistPsi2Compute(sKern, vardist, Z);
...
function [gKern, gVarmeans, gVarcovars, gInd] = rbfard2VardistPsi2GradientOrig(sKern, vardist, Z, covGrad, learnInducing)
% variational means
N = size(vardist.means,1);
%  inducing variables 
[M, Q] = size(Z); 
